...
export async function GET(){
  ...
  return json({status:model==="degraded"?"degraded":"ready",database:process.env.DATABASE_URL?"configured":"mock-mode",redis:process.env.REDIS_URL?"configured":"memory-mode",model},{headers:{"cache-control":"no-store"}});
}
